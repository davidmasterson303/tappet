...
function SettingsSection({
  ...
  title: string;
  /**
   * Optional since 11 Sep, and now unused by every group. Each carried a
   * line restating its heading — "How you appear in the app." under PROFILE
   * — and the critique named the habit: explaining a label that already
   * explains itself. The delete group's went last; its paragraph beneath
   * already states the consequence. The prop stays for a group that has
   * something to say that its title does not.
   */
  description?: string;
  index: number;
  ...
  */
  ...
  async function handleSave() {
    ...
    if (result.success) {
      ...
      // The garage's eyebrow reads the profile through this key (see
      // app/garage/page.tsx); a saved name should show on the next render
      // there rather than after the five-minute stale window.
  ...
  return (
    <SignedInShell actions={<AddVehicleAction />}>
      {/*
        The landing margin, not a centred column. The page sat in `max-w-3xl
        mx-auto`, which put its headline about 230px to the right of where the
        garage's and add-a-vehicle's begin — the brief's "every page opens
        identically" measured against itself. The shell's container is the
        landing's; the content is 720 wide inside it, exactly where the VIN
        panel sits on the page beside this one.
      */}
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-12 py-14">
