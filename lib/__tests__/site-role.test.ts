...
describe('the banner is not rendered unconditionally', () => {
  ...
  it('reads the gate from the server environment, not a client-exposed one', () => {
    ...
    */
    expect(layout).toContain('process.env.TAPPET_DEMO_SITE');
    ...
    expect(layout).not.toContain('NEXT_PUBLIC_TAPPET_DEMO_SITE');
  });
});
  ...
  it('the generated card branches too, and did not', () => {
    /*
      ...
      *"Live demo with sample vehicles — no signup required"* on **both**
      ...
      `tappet.southmoordigital.com` — the App Store listing's own marketing URL
    ...
    */
    ...
    expect(card).toContain('isDemoSite(process.env.TAPPET_DEMO_SITE ?? process.env.WELLKEPT_DEMO_SITE ?? process.env.CREWCHIEF_DEMO_SITE)');
    ...
    */
    ...
    const branch = rendered.slice(rendered.indexOf('isDemoSite(process.env.TAPPET_DEMO_SITE ?? process.env.WELLKEPT_DEMO_SITE ?? process.env.CREWCHIEF_DEMO_SITE)'));
