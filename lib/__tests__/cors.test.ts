...
describe('everything else gets no headers, which is the refusal', () => {
  ...
  it('refuses a lookalike origin', () => {
    ...
    expect(corsHeadersFor('https://wellkept-demo.davidmasterson.co.evil.example', ORIGINS)).toBeNull();
    expect(corsHeadersFor('http://wellkept-demo.davidmasterson.co', ORIGINS)).toBeNull();
  });
  ...
  it('allows nothing when the allowlist is empty', () => {
    expect(corsHeadersFor('https://wellkept-demo.davidmasterson.co', [])).toBeNull();
  });
