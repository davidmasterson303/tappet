 ...
 *
 * `SUPABASE_SECRET_KEY` only — no fallback to the legacy service_role key,
 * which 401s on this project. See the header of `lib/supabase.ts`.
 */
function serviceClient(): SupabaseClient {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.SUPABASE_SECRET_KEY;
  if (!url) throw new Error('plates: NEXT_PUBLIC_SUPABASE_URL is not set');
  if (!key) throw new Error('plates: SUPABASE_SECRET_KEY is not set (the legacy service_role key is not read)');
