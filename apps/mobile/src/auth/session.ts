 ...
 * "Could not reach Tappet" is emitted for *any* message matching
 ...
 * the app, and no onboarding either — so the only way to become a Tappet
...
export async function signIn(email: string, password: string): Promise<SignInResult> {
  ...
  if (error) {
    ...
    return {
      ok: false,
      error: isNetwork
        ? 'Could not reach Tappet. Check your connection and try again.'
 ...
 * make an unauthenticated screen answer "does this person use Tappet" — the
