 ...
 * only place a person can become a Tappet user at all — the web no longer
...
/*
  Not mocked in `jest.setup.js` because it is Tappet's own module, and that
  ...
  */
  ...
  it('never says whether the address has an account', async () => {
    /*
      ...
      downloads the app can enumerate Tappet's users.
    */
    const user = userEvent.setup();
    const view = await render(<SignInScreen />);
